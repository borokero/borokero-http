...
describe("HTTP API", function () {
  ...
  describe("with auth problems", function () {
    beforeEach(function (done) {
      ...
      settings.authorizeGet = function (subject, packet, callback) {
        if (packet.topic === "unauthorizedGet") {
          callback(null, false);
        } else {
          callback(null, true);
        }
      };
      settings.authorizePut = function (subject, packet, callback) {
        if (packet.topic === "unauthorizedPut") {
          callback(null, false);
        } else {
          callback(null, true);
        }
