...
HTTP.prototype.buildServer = function (opts) {
  ...
  function handleGetResource(subject, topic, req, res) {
    ...
    authorizeGet(subject, topicGet, function (err, authorized) {
      if (err) {
      ...
      const lookup = persistence.createRetainedStream(topicGet.topic);
      lookup.pipe(
  ...
  function handlePutResource(subject, topic, req, res) {
    ...
    req.pipe(
      callback(function (err, payload) {
        ...
        authorizePut(subject, packet, function (err, authorized) {
          if (err) {
            handleAuthError(err, res);
            return;
          }
          ...
          persistence.storeRetained(packet, function () {
            res.setHeader("Location", "/resources/" + packet.topic);
            res.statusCode = 204;
            res.end();
            brokero.emit("retained", packet.topic, new Buffer(packet.payload));
          });
        });
      })
    );
 ...
 */
HTTP.prototype.authorizeGet = function (subject, get, cb) {
  cb(null, true);
};
 ...
 * @param {Function} cb The callback function. Has the following structure: cb(err, authorized)
 */
HTTP.prototype.authorizePut = function (subject, packet, cb) {
  cb(null, true);
};
